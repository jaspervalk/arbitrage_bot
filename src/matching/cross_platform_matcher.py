...
async def search_polymarket(base_url: str, query: str, events_tag: str = None) -> List[Dict[str, Any]]:
    # Build query params
    params = [f"q={query}", "keep_closed_markets=0"]
    if events_tag:
        params.append(f"events_tag={events_tag}")
    url = f"{base_url}/public-search?{'&'.join(params)}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            resp.raise_for_status()
            data = await resp.json()
...
async def cross_platform_match(kalshi_base_url: str, polymarket_base_url: str, category: str = None) -> List[Dict[str, Any]]:
    ...
    for series in kalshi_series:
        ...
        if not keywords:
            continue
        # Use Kalshi category as events_tag for Polymarket search
        events_tag = series.get("category", None)
        polymarket_markets = await search_polymarket(polymarket_base_url, keywords, events_tag=events_tag)
